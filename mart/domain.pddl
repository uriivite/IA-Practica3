...
(:action translate
  ...
  :precondition (and (or (aparcat ?r ?b) (l_rover ?r ?a0)) (> (?c 1)))
  :effect (and (when (aparcat ?r ?b) (not (aparcat ?r ?b)) ) (when (l_rover ?r ?a0)
            (not (l_rover ?r ?a0)) ) (l_rover ?r ?a1) (comb_rest ?r (- ?c 1)) )
)
...
:effect (and (not (l_persona ?p ?a)) (rover_1p ?r ?p) (when (rover_buit ?r) (not (rover_buit ?r)))
          (when (rover_1p ?r ?p2) rover_2p ?p ?p2) )
)

(:action pick2person		;same assentament
:parameters (?x - rover ?p1 - persona ?p2 - persona ?a - assentament)
...
:parameters(?x - rover ?s - suministre ?a - assentament)
...
(:action descarregar
  ...
  :effect (when (and (rover_1p ?r ?p1) (peticio_p ?p1 ?a))
            (and (not (rover_1p ?r ?p1))
                (deixo_p ?p1 ?a)
            )
          )
          (when (and (rover_1p ?r ?p2) (peticio_p ?p2 ?a))
